...
object HbaseTool {
  ...
  def createTable(tableName: String, columnFamily: String): Unit = {
    ...
    if(! admin.isTableAvailable(tableName)){
      print("Table not Exists! Creat Table")
  ...
  def putValue(tableName: String, rowKey: String, family: String, qualifierValue: Array[(String, String)]) = {
    ...
    qualifierValue.map(x => {
      ...
      if(!(value.isEmpty)) new_row.add(Bytes.toBytes(family), Bytes.toBytes(column), Bytes.toBytes(value))
    })
    table.put(new_row)
  }

  def putSingleValue(tableName: String, rowKey: String, family: String, qualifier: String, cValue: String): Unit ={
    val table = getTable(tableName)
    val new_row = new Put(Bytes.toBytes(rowKey))
