...
and from_unixtime(unix_timestamp(concat(data,' ',time))) >= '1970-09-01'
...
select ct.userid, ct.itemid, if(ct.pageviews < 5, round(0.2*ct.pageviews,1),0.9) as page_score,
        timeDecay(datediff(from_unixtime(unix_timestamp(),'yyyy-MM-dd HH:mm:ss'), ct.last_visit_time from
(select userid,itemid,sum(fruits_pageviews) as pageviews, max(fruitr_current_visit) as last_visit_time from etl_acesslog
       where  userid <> '' and userid is not null and itemid<> '' and itemid is not null
       group by userid, itemid) as ct where userid <> '' and userid is not null and itemid <> '' and itemid is not null;
...
insert overwrite table scores select
p.userid, m.itemid, p.view_num_score + p.view_time_score + order_num_score as score from iis_pageview_scores p inner join item i
on p.itemid = i.itemid inner join item_mapping_distinct m on p.itemid = m.itemcharc left outer join order_scores_sum o where
p.userid = o.userid and p.itemid = o.itemcharc
